# this code is what this project is inspired by
...
class WarehouseMSP28Strategy(MSP28Strategy):
...
# (DB2 strategy kept if you want it; types identical; omitted here for brevity)
...
def top_level_routes_and_structures(
    parent_item: int,
    TTL: int = 3600 * 24,
    strategy: Optional[MSP28Strategy] = None,
) -> List[Path]:
    # lock in a non-None local for static analyzers
    strat: MSP28Strategy = strategy if strategy is not None else WarehouseMSP28Strategy()
    ...
    with duckdb.connect() as con:
        top_path = _ensure_cache_rel(con, top_dir, TTL, lambda: strat.materialize_top(con, str(parent_item)))
        ...
        bottom_path = _ensure_cache_rel(con, bot_dir, TTL, lambda: strat.materialize_bottom(con, mid_keys))
